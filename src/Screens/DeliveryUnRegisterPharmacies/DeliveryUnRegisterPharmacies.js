...
export default function DeliveryDashboard({ navigation }) {
    ...
    const dataPass = () => {
        console.log("pressed")
        client.post('/DeliveryAgent/UnregisterFromPharmacy', {
            uid: window.loggedUserId,
            pid: navigation.getParam('key'),

        })
        //navigation.goBack()
        navigation.replace('RegisteredPharmacies',null, null)
    }

    const confirmation = () =>
    Alert.alert(
      "Confimation",
      "Are you sure you want to unregister from company?",
      [
        {
          text: "Cancel",
    ...
    return (
        <View style={globalStyles.fullPage}>
            ...
            <TouchableOpacity
                ...
                <Text style={globalStyles.buttonText}>Unregister</Text>
            </TouchableOpacity>
            <Footer />
        </View>
    )
